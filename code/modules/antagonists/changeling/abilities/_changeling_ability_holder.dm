/datum/abilityHolder/changeling
	...
	proc/on_death(mob/mobref, gibbed)
		...
		if(!gibbed && src.headspider_ready)
			src.activate_headspider()
			return
	...
	proc/activate_headspider()
		var/mob/living/carbon/human/body = src.owner
		if(!isdead(body) || !src.headspider_ready)
			return
		src.headspider_ready = FALSE
		//Changelings' heads pop off and crawl away - but only if they're not gibbed and they prepared it beforehand. Oy vey!
		...
		boutput(HS, "<b class = 'hint'>We released a headspider, we'll need to prepare another.....</b>")
